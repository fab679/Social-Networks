...
function makeAugmentedSchema({ typeDefs, resolvers, schemaDirectives, }) {
    const document = merge_1.mergeTypeDefs(Array.isArray(typeDefs) ? typeDefs : [typeDefs]);
    validation_1.default(document);
    ...
    nodes.forEach((node) => {
        ...
        const queryFields = Object.assign(Object.assign({ OR: `[${node.name}Where!]`, AND: `[${node.name}Where!]` }, node.scalarFields.reduce((res, f) => {
            ...
            if (["String", "ID"].includes(f.typeMeta.name)) {
                if (environment_1.default.NEO4J_GRAPHQL_ENABLE_REGEX) {
        ...
        node.relationFields.forEach((rel) => {
            if (rel.union) {
                ...
                refNodes.forEach((n) => {
                    ...
                    const nodeFieldDeleteInputName = `${node.name}${utils_1.upperFirstLetter(rel.fieldName)}${n.name}DeleteInput`;
            ...
            const nodeFieldDeleteInputName = `${node.name}${utils_1.upperFirstLetter(rel.fieldName)}DeleteInput`;
